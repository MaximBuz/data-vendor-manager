...
# Serializers
from ...serializers.usage import AggregatedUsageByActivitySerializer
...
def aggregated_usage_by_activity_tag(request):
  try:
    # get first filtered and then aggregated data
    aggregated_data = get_filtered_aggregated_data(request, "activity_tag_pk")
  except BaseException as error:
    # Return error if no data can be found
    return Response(status=status.HTTP_200_OK)

  # remove unnecessary columns
  aggregated_data = aggregated_data[["usage_time", "activity_tag_pk", "activity_tag"]]

  # serialize data into JSON
  serializer = AggregatedUsageByActivitySerializer(

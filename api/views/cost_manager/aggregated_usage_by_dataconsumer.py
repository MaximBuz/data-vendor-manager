...
# Serializers
from ...serializers.usage import AggregatedUsageDataconsumerSerializer
...
def aggregated_usage_by_dataconsumer(request):
  try:
    # get first filtered and then aggregated data as a pandas dataframe
    aggregated_data = get_filtered_aggregated_data(request, "data_consumer_pk")
  except BaseException as error:
    # Return error if no data can be found
    return Response(status=status.HTTP_200_OK)

  # serialize data into JSON
  serializer = AggregatedUsageDataconsumerSerializer(aggregated_data.to_dict(orient="records"),
                                                     many=True)

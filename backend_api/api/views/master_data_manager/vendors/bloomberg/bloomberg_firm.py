...
def bloomberg_firm(request, firm_nr_id):
    # find object in database
    try:
        firm_nr = BloombergFirm.objects.get(pk=firm_nr_id)
    except BloombergFirm.DoesNotExist:
    ...
    if request.method == "GET":
        ...
        serializer = BloombergFirmSerializer(
            firm_nr, context={'depth': depth})
        return Response(serializer.data)

    # update one location by id
    if request.method == "PATCH":
        serializer = BloombergFirmSerializer(
            firm_nr, data=request.data, partial=True)
